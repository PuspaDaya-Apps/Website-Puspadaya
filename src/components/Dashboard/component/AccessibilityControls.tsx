...
const AccessibilityControls: React.FC<AccessibilityControlsProps> = ({
  ...
  const [position, setPosition] = useState({ top: 140 });
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className="fixed right-0 z-40 mr-14 mt-4 w-80 rounded-lg bg-white p-4 shadow-2xl transition-all duration-300 ease-in-out border-2 border-gray-300"
          style={{ top: `${position.top + 64}px` }}
        >
          ...
          {/* High Contrast Toggle - Only visible when Senior Mode is active */}
          {isSeniorMode && (
            <button
          ...
          {/* Font Size Selector - Only visible when Senior Mode is active */}
          {isSeniorMode && (
          ...
          <div className="mt-3 rounded-lg bg-blue-50 p-2 text-center">
            ...
            <p className="mt-1 text-xs text-gray-500">
              Preview laporan sebelum download PDF
            </p>
          </div>
        </div>
      )}
